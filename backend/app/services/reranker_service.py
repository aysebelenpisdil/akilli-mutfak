...
class RerankerService:
    ...
    def _prepare_recipe_text(self, recipe: Recipe) -> str:
        ...
        Uses Turkish labels to match the language of query and recipe content.
        Instructions are kept (truncated) so the cross-encoder has full context.
        """
        title = recipe.Title or ""

        ingredients_text = recipe.Cleaned_Ingredients or recipe.Ingredients or ""
        ingredients_clean = (
            ingredients_text.replace('[', '').replace(']', '').replace("'", '').strip()
        )
        ...
        return f"Tarif: {title}. Malzemeler: {ingredients_clean}. Yapılışı: {instructions}"
    ...
    def _prepare_query_text(self, ingredients: List[str]) -> str:
        ...
        if not ingredients:
            return ""
        return f"Malzemeler: {', '.join(ingredients)}."
