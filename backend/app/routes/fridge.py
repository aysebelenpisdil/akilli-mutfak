...
async def get_ingredients(user: Annotated[dict, Depends(get_current_user)]):
...
async def save_ingredients(request: FridgeRequest, user: Annotated[dict, Depends(get_current_user)]):
