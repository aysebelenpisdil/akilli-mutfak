...
@router.get("/items")
async def get_items(user: Annotated[dict, Depends(get_current_user)]):
    items = await database_service.get_shopping_list(user["id"])
    return {"items": items}


@router.post("/items")
async def save_items(payload: ShoppingListPayload, user: Annotated[dict, Depends(get_current_user)]):
